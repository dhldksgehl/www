...
$(document).ready(function () {
  ...
  $('.btn1').css('background', '#e01822'); //첫번째 불켜
  ...
  function moveg() {
    ...
    $('.mbutton').css('width', '15'); // 버튼 원래의 너비
    $('.btn' + cnt).css('background', '#e01822'); //자신만 불켜
  ...
  $('.mbutton').click(function (event) { //각각의 버튼 클릭시
    ...
    $('.btn' + cnt).css('background', '#e01822'); //자신 버튼만 불켜 
    $('.btn' + cnt).css('width', '30');
    ...
    if (onoff == false) { //중지상태냐??
      onoff = true; //동작~~
      $('.ps').css('background', 'url(images/stop.png) 0 0 no-repeat');
    }
  ...
  $('.ps').click(function () {
    if (onoff == true) { // 타이머가 동작 중이냐??
      ...
      $(this).css('background', 'url(./images/play.png) 0 0 no-repeat'); //js파일에서는 경로의 기준이 html파일이 기준!!
      onoff = false;
    } else { // false 타이머가 중지 상태냐??
      timeonoff = setInterval(moveg, 4000); //play버튼 클릭시  부활
      $(this).css('background', 'url(./images/stop.png) 0 0 no-repeat');
      onoff = true;
    }
  });
